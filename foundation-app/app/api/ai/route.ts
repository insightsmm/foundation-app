...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (task === 'formats') {
      const sys =
        'You suggest short-form video formats for a brand. Output ONLY a JSON array of exactly 5 items {"name":string,"why":string}. ' +
        'name is the format in 2 to 4 words, why is one short line on when it lands. ' +
        VOICE_RULES;
      const raw = await claude(sys, ctx + 'Suggest 5 formats for this niche.', 500);
      return NextResponse.json({ raw });
    }
