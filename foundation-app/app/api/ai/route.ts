...
function brandContext(b: any): string {
  if (!b) return 'No brand details are set yet. Make reasonable assumptions for a local service business. ';
  ...
  if (c.rhythm) u += `Posting rhythm: ${c.rhythm}. `;
  ...
  if (c.proof) u += `Signature proof: ${c.proof}. `;
  if (c.pov) u += `Permission line: ${c.pov}. `;
  return u;
}
...
export async function POST(req: NextRequest) {
  ...
  const { task, brandId, hook, type, start, count, mode } = body as any;
  ...
  try {
    ...
    if (task === 'score3cs') {
      const sys =
        'You score a brand on the 3Cs of branding for a local service business: Clarity, Consistency, Credibility, each an integer 0 to 100. ' +
        'Output ONLY JSON {"clarity":n,"consistency":n,"credibility":n,"note":string}. The note is one short sentence naming the single biggest lever. ' +
        VOICE_RULES;
      const raw = await claude(sys, ctx + 'Score this brand on the 3Cs.', 300);
      return NextResponse.json({ raw });
    }

    if (task === 'formats') {
      const names: string[] = Array.isArray((body as any).names) ? (body as any).names : [];
      const sys =
        'You write one short example hook for each given short-form video format, tailored to this brand and niche. ' +
        'Output ONLY a JSON array of strings, the same length and order as the formats listed. ' +
        'Each string is a scroll-stopping hook under 16 words that fits that format. ' +
        VOICE_RULES;
      const raw = await claude(sys, ctx + 'Formats in order: ' + names.join(' | ') + '. Return the JSON array of example hooks, one per format, in order.', 600);
      return NextResponse.json({ raw });
    }
    ...
    if (task === 'advise') {
      let sys = '';
      if (mode === 'system') {
        sys = 'You design a simple content system for a solo brand: pillars, a weekly cadence, and a repurposing loop. Output 4 to 6 short plain-text lines, each a concrete step. No markdown bullet characters, no preamble. ' + VOICE_RULES;
      } else if (mode === 'production') {
        sys = 'You lay out a one-week production plan for a solo brand: batch shooting, editing, scheduling, and review. Output 4 to 6 short plain-text lines, each a concrete step. No markdown bullet characters, no preamble. ' + VOICE_RULES;
      } else {
        sys = 'You give brief, concrete content guidance for a brand. Output 4 to 6 short plain-text lines. ' + VOICE_RULES;
      }
      const text = await claude(sys, ctx + 'Give the plan.', 500);
      return NextResponse.json({ text: text.trim() });
    }
