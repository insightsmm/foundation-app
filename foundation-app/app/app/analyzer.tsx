...
import { analyzeProfile, remixVideo, type IgMetrics, type IgReport, type RemixResult } from '@/lib/ai-client';
...
export default function Analyzer({ brand, userId, brandId }: { brand: any; userId: string; brandId: string | null }) {
  ...
  const [mode, setMode] = useState<'audit' | 'remix'>('audit');
  const [saved, setSaved] = useState<any[]>([]);

  useEffect(() => { loadSaved(); /* eslint-disable-next-line */ }, []);
  async function loadSaved() {
    if (!userId) return;
    const { data } = await supabase.from('saved_reports').select('id,handle,title,created_at,data').eq('user_id', userId).eq('kind', 'audit').order('created_at', { ascending: false }).limit(20);
    setSaved(data || []);
  }
  async function saveReport() {
    if (!metrics || !report) return;
    await supabase.from('saved_reports').insert({ user_id: userId, brand_id: brandId, kind: 'audit', handle: metrics.handle, title: metrics.fullName || metrics.handle, data: { metrics, report } });
    loadSaved();
  }
  function selectSaved(row: any) {
    if (row?.data?.metrics) setMetrics(row.data.metrics);
    if (row?.data?.report) setReport(row.data.report);
    setErr('');
  }
  ...
  return (
    <div>
      ...
      <h1 className="h-display proof-h1">Audit a profile, or remix a viral video.</h1>
      <p className="lede">Run a full TPO audit on any public Instagram profile, or drop in a viral video and rebuild its structure for your niche with on-screen hooks, a shot list, and a script.</p>

      <div className="toggle" style={{ marginBottom: 18 }}>
        <button className={mode === 'audit' ? 'on' : ''} onClick={() => setMode('audit')}>Profile audit</button>
        <button className={mode === 'remix' ? 'on' : ''} onClick={() => setMode('remix')}>Viral remix</button>
      </div>

      {mode === 'audit' && (<>
      {saved.length > 0 && (
        <div className="saved-list">
          <span className="muted" style={{ fontSize: 12, marginRight: 4 }}>Saved reports:</span>
          {saved.map((s) => (
            <button key={s.id} className="saved-chip" onClick={() => selectSaved(s)}>@{s.handle}<span>{new Date(s.created_at).toLocaleDateString()}</span></button>
          ))}
        </div>
      )}
      ...
      {metrics && report && (
        <>
          <div className="row" style={{ gap: 10, margin: '18px 0', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button className="btn" onClick={saveReport}>Save to sidebar</button>
          ...
          {report.bio_optimization && (
            <div className="card" style={{ marginTop: 16 }}>
              <h3 className="h-display" style={{ fontSize: 19, margin: '0 0 8px' }}>Bio optimization</h3>
              {report.bio_optimization.assessment && <p className="muted" style={{ margin: '0 0 12px', lineHeight: 1.6 }}>{report.bio_optimization.assessment}</p>}
              {(report.bio_optimization.suggestions || []).map((b, i) => <div key={i} className="biopt">{b}</div>)}
            </div>
          )}
      ...
      {mode === 'remix' && <Remix brand={brand} />}
    </div>
  );
}

function Remix({ brand }: { brand: any }) {
  ...
  async function run() {
    if (!script.trim() && !url.trim()) { setErr('Paste the video script, or a reel URL.'); return; }
    if (!niche.trim()) { setErr('Tell me which niche to remix it for.'); return; }
    setLoading(true); setErr(''); setResult(null);
    const res = await remixVideo({ script, url, niche });
    if (res.error) setErr(res.error);
  ...
  function copyAll() {
    ...
    const txt = [
      ...
      result.script?.length ? '\nSCRIPT:\n' + result.script.map((s) => `[${s.label}] ${s.line}`).join('\n') : '',
      result.onscreen_text?.length ? '\nON-SCREEN TEXT:\n' + result.onscreen_text.join('\n') : '',
      result.shot_list?.length ? '\nSHOT LIST:\n' + result.shot_list.map((s) => `- ${s.shot}: ${s.note}`).join('\n') : '',
      result.caption ? '\nCAPTION:\n' + result.caption : '',
      result.cta ? '\nCTA:\n' + result.cta : '',
    ].filter(Boolean).join('\n');
  ...
  return (
    <div>
      <div className="card">
        <label className="lab">Niche to remix for</label>
        <input className="field" value={niche} onChange={(e) => setNiche(e.target.value)} placeholder="family law, roofing, fitness coaching..." />
        <label className="lab" style={{ marginTop: 14, display: 'block' }}>Paste the viral video script or on-screen text</label>
        <textarea className="field" rows={5} value={script} onChange={(e) => setScript(e.target.value)} placeholder="Paste what is said and shown in the video..." />
        <label className="lab" style={{ marginTop: 14, display: 'block' }}>Or a reel URL (pulls the caption)</label>
        <input className="field" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="instagram.com/reel/..." />
        <div style={{ marginTop: 14 }}>
          <button className="btn mark" onClick={run} disabled={loading}>{loading ? 'Remixing...' : 'Remix for my niche'}</button>
        </div>
        {err && <p className="err">{err}</p>}
      </div>
      ...
      {result && (
        <>
          {result.why_it_works && (
            <div className="convo"><div className="convo-l">&#10022; Why it works</div><p className="convo-q" style={{ fontSize: 16 }}>{result.why_it_works}</p></div>
          )}
          {result.structure && result.structure.length > 0 && (
            <div className="card"><label className="lab">The structure</label>
              <div style={{ marginTop: 8 }}>{result.structure.map((s, i) => (
                <div key={i} className="plan-line"><span className="recnum" style={{ width: 22, height: 22 }}>{i + 1}</span><span><b>{s.beat}.</b> <span className="muted">{s.purpose}</span></span></div>
              ))}</div>
            </div>
          )}
          {result.hooks && result.hooks.length > 0 && (
            <div className="card"><label className="lab">On-screen hook options</label>
              {result.hooks.map((h, i) => <div key={i} className="hookline"><span className="mk">{h}</span></div>)}
            </div>
          )}
          {result.script && result.script.length > 0 && (
            <div className="card"><label className="lab">Your script</label>
              <div style={{ marginTop: 8 }}>{result.script.map((s, i) => (
                <div key={i} className="beat"><div className="bl">{s.label}</div><p>{s.line}</p></div>
              ))}</div>
            </div>
          )}
          {result.onscreen_text && result.onscreen_text.length > 0 && (
            <div className="card"><label className="lab">On-screen text, in order</label>
              <div className="chips" style={{ marginTop: 8 }}>{result.onscreen_text.map((t, i) => <span key={i} className="chip">{t}</span>)}</div>
            </div>
          )}
          {result.shot_list && result.shot_list.length > 0 && (
            <div className="card"><label className="lab">Shot list</label>
              <ul className="clist" style={{ marginTop: 8 }}>{result.shot_list.map((s, i) => <li key={i}><b>{s.shot}.</b> {s.note}</li>)}</ul>
            </div>
          )}
          {(result.caption || result.cta) && (
            <div className="card">
              {result.caption && (<><label className="lab">Caption</label><p style={{ margin: '6px 0 0', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{result.caption}</p></>)}
              {result.cta && (<div style={{ marginTop: 12 }}><label className="lab">CTA</label><p style={{ margin: '6px 0 0' }}><span className="mk">{result.cta}</span></p></div>)}
            </div>
          )}
          <button className="btn" onClick={copyAll} style={{ marginTop: 4 }}>Copy everything</button>
        </>
      )}
    </div>
  );
...
.biopt{border:1px solid #EBE3D3;border-radius:10px;padding:12px 14px;margin-bottom:8px;font-size:14px;background:#FBF6E9}
...
${r.bio_optimization ? `<div class="card"><h3 style="font-size:19px;margin-bottom:8px">Bio optimization</h3>${r.bio_optimization.assessment ? `<p style="color:#4a463f;margin:0 0 12px">${esc(r.bio_optimization.assessment)}</p>` : ''}${(r.bio_optimization.suggestions || []).map((b: any) => `<div class="biopt">${esc(b)}</div>`).join('')}</div>` : ''}
