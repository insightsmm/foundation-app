...
import { generateMonth, generateWeek, quotaStatus, consumeQuota, rewritePost, type Post, type Quota } from '@/lib/ai-client';
...
export default function Engine({ brandId, userId, brand }: { brandId: string | null; userId: string; brand: any }) {
  ...
  useEffect(() => { if (brandId) quotaStatus(brandId).then(setQuota).catch(() => {}); /* eslint-disable-next-line */ }, []);
  ...
  async function generate() {
    ...
    const span: '30' | '7' = view === 'week' ? '7' : '30';
    setErr(''); setLiveNote('');
    const q = await quotaStatus(brandId).catch(() => null);
    if (q) {
      setQuota(q);
      const can = span === '30' ? q.can30 : q.can7;
      if (!can) {
        setErr(span === '30'
          ? `You have used your 30-day generation for this brand this billing period. The next one unlocks at renewal${q.periodEnd ? ' on ' + new Date(q.periodEnd).toLocaleDateString() : ''}.`
          : 'You have used all 4 weekly generations for this brand this month. They reset on the 1st.');
        return;
      }
    }
    setStreaming(true); setShownRows(0);
    STREAM_ROWS.forEach((_, i) => setTimeout(() => setShownRows((n) => Math.max(n, i + 1)), i * 340));
    let live = true;
    let got: Post[] = [];
    try { got = span === '7' ? await generateWeek(brandId) : await generateMonth(brandId); } catch { live = false; }
    ...
    if (live && got.length) {
      ...
      await load();
      const after = await consumeQuota(brandId, span).catch(() => null);
      if (after) setQuota(after);
      if (after && after.isAdmin) {
        setLiveNote('\u2713 Saved. Admin access, no generation limit applies.');
      } else if (span === '30') {
        setLiveNote(`\u2713 Saved. That is your one 30-day calendar for this billing period${after && after.periodEnd ? ', renews ' + new Date(after.periodEnd).toLocaleDateString() : ''}. Weekly top-ups used: ${after ? after.used7 : 0} of 4.`);
      } else {
        const left = after ? Math.max(0, after.allowed7 - after.used7) : 0;
        setLiveNote(`\u2713 Saved. You have ${left} of 4 weekly generations left this month for this brand.`);
      }
    } else {
    ...
    <div>
      ...
      {quota && !quota.isAdmin && (
        <p className="muted" style={{ fontSize: 12, margin: '0 0 12px' }}>
          30-day calendar: {quota.used30}/{quota.allowed30} used this billing period &middot; Weekly calendars: {quota.used7}/{quota.allowed7} used this month
        </p>
      )}
      {quota && quota.isAdmin && (
        <p className="muted" style={{ fontSize: 12, margin: '0 0 12px' }}>Admin access, no generation limit.</p>
      )}
