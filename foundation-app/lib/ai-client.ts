...
export type TpoRating = { rating: string; note: string };
export type IgReport = {
  summary: string;
  clarity_verdict?: string;
  scores: { clarity: number; consistency: number; credibility: number };
  score_notes?: { clarity?: string; consistency?: string; credibility?: string };
  tpo?: { teach?: TpoRating; proof?: TpoRating; offer?: TpoRating; balance?: string };
  pillars: string[];
  gaps?: string[];
  opportunities?: { title: string; detail: string }[];
  sample_posts?: { type: string; hook: string; setup: string; punch: string; land: string; cta: string }[];
  action_plan?: string[];
};
