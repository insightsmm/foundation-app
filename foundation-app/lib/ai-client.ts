...
export type IgReport = {
  ...
  bio_optimization?: { assessment?: string; suggestions?: string[] };
...
export type RemixResult = {
  ...
  why_it_works?: string;
  hooks?: string[];
  script?: { label: string; line: string }[];
...
export async function remixVideo(input: { url?: string; script?: string; niche?: string }): Promise<{ result?: RemixResult; source?: string; error?: string }> {
  const res = await fetch('/api/remix', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) return { error: data?.error || ('Request failed (' + res.status + ')') };
  return data;
}
...
export type Quota = {
  isAdmin: boolean; plan: string | null; periodEnd: string | null;
  used30: number; allowed30: number; can30: boolean;
  used7: number; allowed7: number; can7: boolean;
  ok?: boolean; message?: string;
};

export async function quotaStatus(brandId: string): Promise<Quota> {
  const res = await fetch('/api/quota', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ brandId, action: 'status' }) });
  return res.json();
}
export async function consumeQuota(brandId: string, span: '30' | '7'): Promise<Quota> {
  const res = await fetch('/api/quota', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ brandId, action: 'consume', span }) });
  return res.json();
}
export async function generateWeek(brandId: string): Promise<Post[]> {
  const r = await callAI({ task: 'generate', brandId, start: 1, count: 7 });
  const merged = parseJsonLoose<any[]>(r.raw, '[', ']', []);
  return normalize(merged).slice(0, 7).map((p, i) => ({ ...p, day: i + 1 }));
}
